...
En este ejercicio, añadiremos la funcionalidad de gestión de logros y recompensas para los jugadores. Concretamente, se proporciona una clase “Achievement” que representa los distintos logros que los usuarios pueden conseguir en sus partidas. Un logro tiene una descripción que detalla su objetivo, un valor numérico necesario para su completitud y una categoría a la que pertenece (por ejemplo, "Victorias", "Tácticas" o "Estrategia"). Además, tendremos la clase “UserAchievement” que registra el progreso individual de cada usuario hacia un logro específico. Esta clase indicará el progreso actual del usuario, si el logro ha sido completado y, en caso afirmativo, la fecha en que se desbloqueó.

Los usuarios del sistema pueden tener múltiples instancias de `UserAchievement` asociadas, reflejando su avance o consecución de distintos logros. Del mismo modo, cada `Achievement` puede tener asociados múltiples registros de `UserAchievement`, uno por cada usuario que esté persiguiendo o haya completado ese logro.
