...
export const GithubService = {
  ...
  async createOrUpdateFile(
    ...
    try {
      ...
      console.log(
        `El archivo ${path} no existe previamente, se creará uno nuevo.`
      )
    }
  ...
  async deployExam(
    ...
    const newRepo = await this.createRepoFromTemplate(
      token,
      templateOwner,
      templateRepo,
      newRepoName
    ...
    await this.updateReadmeWithDescription(
      token,
      TARGET_OWNER,
      newRepoName,
      markdownContent
    ...
    if (project?.fullSolution?.trim() && project?.baseClasses) {
      await this._uploadSolutionBranch(
        token,
        TARGET_OWNER,
        newRepoName,
        project.baseClasses,
  ...
  async createOrUpdateFileOnBranch(
    ...
    const base64Content = btoa(
      encodeURIComponent(content).replaceAll(/%([0-9A-F]{2})/g, (_, p1) =>
    ...
    if (!response.ok) {
      const errorData = await response.json()
      throw new Error(
        `Error subiendo ${path} a rama ${branch}: ${errorData.message}`
      )
    }
