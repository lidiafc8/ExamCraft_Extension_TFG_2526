...
vi.mock("~src/screens/codeGeneration/GenerationBaseClassesScreen", () => ({
  default: ({
    ...
    <div data-testid="screen-generateBaseClasses">
      <span data-testid="initial-project">{JSON.stringify(initialProject)}</span>
...
vi.mock("~src/screens/codeGeneration/GenerationSolutionCodeScreen", () => ({
  default: ({ onBack, onWelcome, onCreateExam, onCreateExamByParts, onCodeGeneration }: any) => (
...
vi.mock("../screens/examStatementGeneration/DomainSelectionScreen", () => ({
  default: ({ onBack, onWelcome, onSelectDomain, onCreateExam, onCreateExamByParts }: any) => (
    <div data-testid="screen-domainSelection">
      ...
      <button onClick={() => onSelectDomain("veterinaria")}>Seleccionar Dominio</button>
      <button onClick={onCreateExam}>CreateExam desde DomainSelection</button>
      <button onClick={onCreateExamByParts}>ByParts desde DomainSelection</button>
    </div>
  )
}))
...
vi.mock("../screens/examStatementGeneration/ContextWorkflowScreen", () => ({
  default: ({
    ...
    <div data-testid="screen-contextWorkflow">
      ...
      <button onClick={onCreateExamByParts}>ByParts desde ContextWorkflow</button>
      <button onClick={onFunctionalExtension}>Extensión desde ContextWorkflow</button>
      <button onClick={() => onCreateDiagram("contexto generado")}>Crear Diagrama</button>
...
vi.mock("../screens/examStatementGeneration/DiagramaUMLWorkflowScreen", () => ({
  default: ({
    ...
    <div data-testid="screen-diagramUML">
      ...
      <button onClick={onFunctionalExtension}>Extensión desde DiagramUML</button>
      <button onClick={onStatementStep1}>Step1 desde DiagramUML</button>
      <button onClick={() => onFinishExtension("enunciado final", "diagrama mermaid")}>
        Finalizar Extensión
      </button>
...
vi.mock("../screens/examStatementGeneration/FinishFunctionalExtensionScreen", () => ({
  default: ({
    onBack,
    onWelcome,
    onCreateExam,
    ...
    <div data-testid="screen-finishFunctionalExtension">
      ...
      <button onClick={onCreateExamByParts}>ByParts desde FinishExtension</button>
      <button onClick={onFunctionalExtension}>Extensión desde FinishExtension</button>
      <button onClick={onStatementStep1}>Step1 desde FinishExtension</button>
      <button onClick={onComponents}>Componentes desde FinishExtension</button>
    </div>
  )
}))
...
vi.mock("~src/screens/examStatementGeneration/EntityRelationshipsWorkflowScreen", () => ({
  default: ({
    ...
    <div data-testid="screen-entityRelationships">
      ...
      <button onClick={onCreateExamByParts}>ByParts desde Relationships</button>
      <button
        onClick={() =>
...
vi.mock("../screens/codeGeneration/GenerationTestsScreen", () => ({
  default: ({
    ...
    <div data-testid="screen-testAttributes">
      ...
      <button onClick={onCreateExamByParts}>ByParts desde TestAttributes</button>
...
describe("IndexTab", () => {
  ...
  describe("Navegación hacia atrás (botones Volver)", () => {
    it("vuelve a Welcome desde GitHub", async () => {
      render(<IndexTab />)
      await userEvent.click(screen.getByRole("button", { name: "Ir a GitHub" }))
      await userEvent.click(screen.getByRole("button", { name: "Volver desde GitHub" }))
      expect(screen.getByTestId("screen-welcome")).toBeInTheDocument()
    })
    ...
    it("vuelve a Welcome desde CreateExam", async () => {
      render(<IndexTab />)
      await userEvent.click(screen.getByRole("button", { name: "Crear Examen" }))
      await userEvent.click(screen.getByRole("button", { name: "Volver desde CreateExam" }))
      expect(screen.getByTestId("screen-welcome")).toBeInTheDocument()
    })
    ...
    it("vuelve a CreateExam desde CreateExamByParts", async () => {
      ...
      await userEvent.click(screen.getByRole("button", { name: "Volver desde ByParts" }))
      expect(screen.getByTestId("screen-createExam")).toBeInTheDocument()
    })
    ...
    it("vuelve a Welcome desde Storage", async () => {
      render(<IndexTab />)
      await userEvent.click(screen.getByRole("button", { name: "Ver Storage" }))
      await userEvent.click(screen.getByRole("button", { name: "Welcome desde Storage" }))
      expect(screen.getByTestId("screen-welcome")).toBeInTheDocument()
    })
    ...
    it("vuelve a CreateExamByParts desde StatementPartSelection", async () => {
      ...
      await userEvent.click(screen.getByRole("button", { name: "Volver desde StatementPart" }))
    ...
    it("vuelve a CodeGeneration desde GenerationSolutionCode", async () => {
      ...
      await userEvent.click(screen.getByRole("button", { name: "Generar Solución" }))
      await userEvent.click(screen.getByRole("button", { name: "Volver desde SolutionCode" }))
  ...
  describe("Flujo completo: creación por partes y generación de código", () => {
    ...
    it("navega CodeGeneration → TestGeneral al pulsar Generar Test", async () => {
      ...
      await userEvent.click(screen.getByRole("button", { name: "Por Partes" }))
      await userEvent.click(screen.getByRole("button", { name: "Código desde ByParts" }))
      await userEvent.click(screen.getByRole("button", { name: "Generar Test" }))
  ...
  describe("Flujo: Atributos → Clases Base → Tests", () => {
    ...
    it("onGoToTests desde BaseClasses (via Attributes) lleva a testAttributes con source 'attributes'", async () => {
      ...
      expect(screen.getByTestId("test-initial-data")).toHaveTextContent("constraints")
    })
    ...
    it("Volver desde testAttributes (origen attributes) regresa a attributesConstraints", async () => {
      ...
      await userEvent.click(screen.getByRole("button", { name: "Volver desde TestAttributes" }))
      expect(screen.getByTestId("screen-attributesConstraints")).toBeInTheDocument()
    })
  })
  ...
  describe("Flujo: Relaciones entre Entidades → Clases Base → Tests", () => {
    ...
    it("navega a GenerateBaseClasses desde Relationships con fromAttributes=true", async () => {
      await llegarAEntityRelationships()
      await userEvent.click(
        screen.getByRole("button", { name: "Ir a Clases Base desde Relationships" })
      )
      expect(screen.getByTestId("screen-generateBaseClasses")).toBeInTheDocument()
      expect(screen.getByTestId("from-attributes")).toHaveTextContent("true")
    })
    ...
    it("onGoToTests desde BaseClasses (via Relationships) lleva a testAttributes con source 'entityRelationships'", async () => {
      ...
      await userEvent.click(screen.getByRole("button", { name: "Ir a Tests" }))
      expect(screen.getByTestId("screen-testAttributes")).toBeInTheDocument()
      expect(screen.getByTestId("test-source")).toHaveTextContent("entityRelationships")
    })
  ...
  describe("Flujo: Extensión Funcional completo", () => {
    ...
    it("'Crear Diagrama' desde ContextWorkflow navega a DiagramUML con contexto", async () => {
      ...
      expect(screen.getByTestId("diagram-context")).toHaveTextContent("contexto generado")
      expect(screen.getByTestId("diagram-domain")).toHaveTextContent("veterinaria")
    })
    ...
    it("'Finalizar Extensión' desde DiagramUML navega a FinishFunctionalExtension con datos correctos", async () => {
      ...
      expect(screen.getByTestId("screen-finishFunctionalExtension")).toBeInTheDocument()
      expect(screen.getByTestId("finish-domain")).toHaveTextContent("veterinaria")
    ...
    it("vuelve a DiagramUML desde FinishFunctionalExtension", async () => {
      ...
      await userEvent.click(screen.getByRole("button", { name: "Crear Diagrama" }))
      await userEvent.click(screen.getByRole("button", { name: "Finalizar Extensión" }))
      await userEvent.click(screen.getByRole("button", { name: "Volver desde FinishExtension" }))
      expect(screen.getByTestId("screen-diagramUML")).toBeInTheDocument()
    })
    ...
    it("'Extensión desde DiagramUML' navega a DomainSelection", async () => {
      ...
      await userEvent.click(screen.getByRole("button", { name: "Seleccionar Dominio" }))
      await userEvent.click(screen.getByRole("button", { name: "Crear Diagrama" }))
      await userEvent.click(screen.getByRole("button", { name: "Extensión desde DiagramUML" }))
      expect(screen.getByTestId("screen-domainSelection")).toBeInTheDocument()
    })
    ...
    it("'Step1 desde DiagramUML' regresa a ContextWorkflow", async () => {
      ...
      await userEvent.click(screen.getByRole("button", { name: "Seleccionar Dominio" }))
      await userEvent.click(screen.getByRole("button", { name: "Crear Diagrama" }))
      await userEvent.click(screen.getByRole("button", { name: "Step1 desde DiagramUML" }))
      expect(screen.getByTestId("screen-contextWorkflow")).toBeInTheDocument()
    })
  })
  ...
  describe("Atajos de navegación global", () => {
    it("Welcome desde cualquier pantalla profunda lleva a welcome", async () => {
      ...
      await userEvent.click(screen.getByRole("button", { name: "Por Partes" }))
      await userEvent.click(screen.getByRole("button", { name: "Componentes" }))
      await userEvent.click(screen.getByRole("button", { name: "Welcome desde StatementPart" }))
      expect(screen.getByTestId("screen-welcome")).toBeInTheDocument()
    })
    ...
    it("ByParts desde CodeGeneration lleva a createExamByParts", async () => {
      ...
      await userEvent.click(screen.getByRole("button", { name: "Por Partes" }))
      await userEvent.click(screen.getByRole("button", { name: "Código desde ByParts" }))
      await userEvent.click(screen.getByRole("button", { name: "ByParts desde CodeGen" }))
      expect(screen.getByTestId("screen-createExamByParts")).toBeInTheDocument()
    })
    ...
    it("Welcome desde ByParts lleva a welcome", async () => {
      ...
      await userEvent.click(screen.getByRole("button", { name: "Crear Examen" }))
      await userEvent.click(screen.getByRole("button", { name: "Por Partes" }))
      await userEvent.click(screen.getByRole("button", { name: "Welcome desde ByParts" }))
      expect(screen.getByTestId("screen-welcome")).toBeInTheDocument()
    })
    ...
    it("CodeGen desde TestAttributes lleva a codeGeneration", async () => {
      ...
      await userEvent.click(screen.getByRole("button", { name: "Código desde ByParts" }))
      await userEvent.click(screen.getByRole("button", { name: "Generar Test" }))
      await userEvent.click(screen.getByRole("button", { name: "Crear Test1 desde TestGeneral" }))
      await userEvent.click(screen.getByRole("button", { name: "CodeGen desde TestAttributes" }))
      expect(screen.getByTestId("screen-codeGeneration")).toBeInTheDocument()
    })
    ...
    it("Componentes desde ContextWorkflow lleva a statementPartSelection", async () => {
      ...
      await userEvent.click(screen.getByRole("button", { name: "Seleccionar Dominio" }))
      await userEvent.click(
        screen.getByRole("button", { name: "Componentes desde ContextWorkflow" })
      )
      expect(screen.getByTestId("screen-statementPartSelection")).toBeInTheDocument()
    })
  })
  ...
  describe("Casos límite y estados compartidos", () => {
    ...
    it("volver a DomainSelection desde ContextWorkflow resetea flujo de extensión", async () => {
      ...
      await userEvent.click(screen.getByRole("button", { name: "Extensión Funcional" }))
      await userEvent.click(screen.getByRole("button", { name: "Seleccionar Dominio" }))
      await userEvent.click(screen.getByRole("button", { name: "Volver desde ContextWorkflow" }))
      expect(screen.getByTestId("screen-domainSelection")).toBeInTheDocument()
    })
