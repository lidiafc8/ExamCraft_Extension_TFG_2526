...
vi.mock("~src/utils/promptParser", () => ({
  parseMasterPrompt: vi.fn(() => ({
    visibleText: "Prompt: {enunciado_restricciones} | {enunciado_relaciones} | {codigo_tests_restricciones} | {codigo_tests_relaciones} | {codigo_base_localstorage}",
  ...
  Header: ({ onWelcome, breadcrumbItems, currentStep }: any) => (
    <header data-testid="header-mock">
      ...
      {breadcrumbItems?.map((bi: any) => (
        <button key={bi.label} onClick={bi.action}>{bi.label}</button>
      ))}
    </header>
  )
}))
...
vi.mock("~src/components/WorkflowComponents", () => ({
  PromptEditor: ({ onGenerate, onBack, description, isLoading }: any) => (
    <div data-testid="prompt-editor">
      ...
      {isLoading && <span data-testid="loading-spinner">Generando solución...</span>}
...
vi.mock("~src/components/modals/ConfirmModal", () => ({
  ConfirmModal: ({ title, message, warning, onConfirm, onCancel, confirmLabel, cancelLabel }: any) => (
...
describe("Integración Completa: GenerationSolutionCodeScreen", () => {
  ...
  describe("Casos Positivos y Navegación", () => {
    ...
    it("abre el modal de confirmación al seleccionar un proyecto y permite avanzar al workflow", async () => {
      ...
      const btnProyecto = await screen.findByRole("button", { name: /Seleccionar Examen Ajedrez Pro/i })
      await userEvent.click(btnProyecto)

      expect(screen.getByTestId("confirm-modal")).toBeInTheDocument()
      expect(screen.getByText(/¿Deseas generar el código solución para el examen Examen Ajedrez Pro?/i)).toBeInTheDocument()
    ...
    it("comporta el flujo completo de generación de IA hasta visualizar el resultado", async () => {
      ...
      await userEvent.click(await screen.findByRole("button", { name: /Seleccionar Examen Ajedrez Pro/i }))
      await userEvent.click(screen.getByRole("button", { name: "Confirmar" }))

      expect(screen.getByText("Enunciado de Restricciones de Atributos")).toBeInTheDocument()
    ...
    it("permite guardar la solución exitosamente en Chrome Storage y transicionar al éxito", async () => {
      ...
      await waitFor(() => {
        expect(saveToChrome).toHaveBeenCalledWith("project_1", expect.objectContaining({
    ...
    it("ejecuta la descarga del Markdown interactuando con DownloadConfirmModal", async () => {
      ...
      const botonesDescargar = await screen.findAllByRole("button", { name: "Descargar (.md)" })
      await userEvent.click(botonesDescargar[0]) 

      const botonConfirmarModal = (await screen.findAllByRole("button", { name: "Descargar (.md)" }))[1]
    ...
    it("permite retroceder en los distintos pasos de la interfaz", async () => {
      ...
      await userEvent.click(await screen.findByRole("button", { name: "Volver Atras" }))
      expect(defaultProps.onBack).toHaveBeenCalled()

      await userEvent.click(await screen.findByRole("button", { name: /Seleccionar Examen Ajedrez Pro/i }))
      await userEvent.click(screen.getByRole("button", { name: "Cancelar Accion" }))
      expect(screen.queryByTestId("confirm-modal")).not.toBeInTheDocument()

      await userEvent.click(await screen.findByRole("button", { name: /Seleccionar Examen Ajedrez Pro/i }))
      await userEvent.click(screen.getByRole("button", { name: "Confirmar" }))
      await userEvent.click(screen.getByRole("button", { name: "Atrás Selección" }))
      expect(screen.getByTestId("folder-selector")).toBeInTheDocument()
    })
  })
  ...
  describe("Casos Negativos y Ramas de Excepciones", () => {
    ...
    it("ejecuta por completo la estructura de excepción al fallar el guardado (Líneas 125-130)", async () => {
      vi.mocked(saveToChrome).mockRejectedValue({ message: "Error de consistencia" }) 
    ...
    it("renderiza el error explícito con su mensaje si la excepción de guardado lo incluye", async () => {
      vi.mocked(saveToChrome).mockRejectedValueOnce({ message: "Storage lleno" })
    ...
    it("asigna un arreglo vacío a los proyectos si la promesa getAllFromChrome falla", async () => {
      vi.mocked(getAllFromChrome).mockRejectedValue(new Error("Fallo de lectura"))
      render(<GenerationSolutionCodeScreen {...defaultProps} />)
  ...
  describe("Casos Límite y Ramas de Lógica", () => {
    it("valida el filtro de proyectos combinando múltiples estados incompletos (Función filterProject)", async () => {
      const proyectosInvalidos = [
        ...
        { _key: "project_B", baseClasses: "class A {}", attributeConstraints: "", entityRelationships: "" }, 
        { _key: "project_C", baseClasses: "class A {}", attributeConstraints: "Tengo restricciones", testPartsMap: {} } 
      ]
    ...
    it("aplica los reemplazos alternativos en buildPrompt si las propiedades opcionales del proyecto vienen vacías", async () => {
      ...
      await userEvent.click(await screen.findByRole("button", { name: /Seleccionar clínica veterinaria/i }))
      await userEvent.click(screen.getByRole("button", { name: "Confirmar" }))

      expect(screen.getByText("Enunciado de Relaciones entre Entidades")).toBeInTheDocument()
      expect(screen.queryByText("Enunciado de Restricciones de Atributos")).not.toBeInTheDocument()
    })
    ...
    it("incluye el warning de sobreescritura si el proyecto seleccionado ya contenía una solución previa", async () => {
      ...
      expect(screen.getByText("Este examen ya tiene un código solución. Si continúas, se reemplazará por la nueva versión.")).toBeInTheDocument()
    })
