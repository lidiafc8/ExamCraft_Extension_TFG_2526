...
export default function SelectionGenerationTestScreen({
        ...
        const projectList = allItems
          ...
          .filter((p) => {
            ...
            const hasConstraints =
              ...
              p.attributeConstraints.trim().length > 10
            const hasRelationships =
              p.entityRelationships && p.entityRelationships.trim().length > 10
  ...
  const allowedFolders = Array.from(
    new Set(
      projects
        ...
        .flatMap((p) =>
          Array.isArray(p.domainName) ? p.domainName : [p.domainName]
        )
  ...
  const handleConfirm = () => {
    ...
    onCreateTest1({
      ...
      targetType:
        key === "attributeConstraints" ? "attributes" : "entityRelationships"
    })
  }
  ...
  return (
    <div className="exam-app">
      ...
      {showPartConfirmModal &&
        partModalData &&
        createPortal(
          <ConfirmModal
            title={
              partModalData.hasExistingTests
                ? "Confirmar Examen"
                : "Confirmar Parte"
            }
