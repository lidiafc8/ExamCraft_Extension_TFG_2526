...
let promptVisibleSimulado =
  "Genera las clases base en Java para el dominio clínica veterinaria. Clases a incluir: "
...
vi.mock("~src/components/Header", () => ({
  Header: (props: any) => {
    capturedBreadcrumbs =
      props.breadcrumbs || props.breadcrumbItems || props.items
    return (
...
describe("GenerationBaseClassesScreen - Pruebas Integrales", () => {
  ...
  it("Debe saltar directamente al flujo de trabajo si se provee un 'initialProject'", async () => {
    render(
      <GenerationBaseClassesScreen
        {...defaultProps}
        initialProject={mockProjects.project_1}
      />
    )
    const promptDescription = screen.getByText(
      /este es el prompt que se usará/i
    )
  ...
  it("Flujo Completo: Seleccionar Carpeta -> Seleccionar Examen -> Confirmar -> Generar código", async () => {
    ...
    const openExamBtn = await screen.findByRole("button", {
      name: /abrir examen/i
    })
    ...
    await waitFor(() => {
      const modalMessage = screen.getByText(
        /¿Deseas utilizar Examen Mascotas como base/i
      )
    ...
    await waitFor(() => {
      const resultTitle = screen.getByText(
        /propuesta del código de las clases bases/i
      )
      expect(resultTitle).toBeDefined()
    })

    expect(
      screen.getByRole("button", { name: /descargar \(.md\)/i })
    ).toBeDefined()
  ...
  it("Debe cerrar el modal de confirmación y limpiar el proyecto seleccionado al hacer click en Cancelar", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(/¿Deseas utilizar Examen Mascotas como base/i)
      ).toBeDefined()
    })
    ...
    await waitFor(() => {
      expect(
        screen.queryByText(/¿Deseas utilizar Examen Mascotas como base/i)
      ).toBeNull()
  ...
  it("Debe mostrar un mensaje de advertencia si el examen ya cuenta con clases base guardadas", async () => {
    ...
    await waitFor(() => {
      const warningText = screen.getByText(
        /este examen ya tiene clases base generadas/i
      )
  ...
  it("Debe abrir el modal de guardado al hacer click en 'Guardar' desde la vista de resultados", async () => {
    ...
    await waitFor(() => {
      ...
      const successDescription = screen.getByText(
        /Las clases base se han guardado correctamente/i
      )
  ...
  it("Debe capturar el error en buildPrompt e inyectar el texto de error por defecto si el proyecto está corrupto", async () => {
    ...
    const corruptProject = {
      id: "project_corrupt",
      domainName: 12345 as any,
      customName: "Proyecto Corrupto"
    }

    render(
      <GenerationBaseClassesScreen
        {...defaultProps}
        initialProject={corruptProject}
      />
    )
    const errorTextarea = (await screen.findByRole(
      "textbox"
    )) as HTMLTextAreaElement

    expect(consoleSpy).toHaveBeenCalledWith(
      expect.stringContaining("Error en buildPrompt:"),
      expect.any(Error)
    )
  ...
  it("Debe inyectar el texto por defecto en el prompt si el dominio del proyecto no está registrado en el diccionario", async () => {
    ...
    const unmappedProject = {
      id: "project_unmapped",
      domainName: "astronomía avanzada",
      customName: "Examen de Estrellas"
    }
    render(
      <GenerationBaseClassesScreen
        {...defaultProps}
        initialProject={unmappedProject}
      />
    )
  ...
  it("Debe construir el prompt estructurado por defecto si visibleText viene vacío", async () => {
    ...
    const textarea = (await screen.findByRole("textbox")) as HTMLTextAreaElement
    expect(textarea.value).toContain(
      "Genera las clases base en Java para el dominio clínica veterinaria."
    )
  })
  ...
  it("Debe estructurar correctamente el log payload mediante buildLogPayload al resolverse la generación", async () => {
    ...
    expect(capturedHookOptions.logExerciseName).toBe(
      "base-classes-code-generation"
    )
