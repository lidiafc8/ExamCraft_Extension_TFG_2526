      ...
      {projectsInFolder.map((p: any) => (
        <div key={p.id} data-testid={`project-item-${p.id}`}>
          ...
          <button onClick={async () => { 
            ...
            await onRenameProject(p.id, "Nuevo Nombre") 
          }}>
            Renombrar
          </button>
        </div>
...
vi.mock("./ExamDetailScreen", () => ({
  ExamDetailScreen: ({ onShowGeneratedCode, onShowSolutionGeneratedCode, onBack, onDownload, onGitHubDeploy, onDeleteProject, selectedProject }: any) => (
    <div data-testid="screen-exam-detail">
      ...
      <button onClick={async () => await onGitHubDeploy("mock-token", selectedProject)}>Desplegar GitHub</button>
...
vi.mock("./GenerationCodeScreen", () => ({
  GeneratedCodeScreen: ({ onBack, onDeleteSection, onDeleteTest, onUpdateProject, selectedProject }: any) => (
    <div data-testid="screen-generated-code">
      ...
      <button onClick={async () => await onDeleteSection("baseClasses")}>Borrar Clases Base</button>
      <button onClick={async () => await onDeleteTest("test-1")}>Borrar Test Individual</button>
      <button onClick={async () => {
        await onUpdateProject({ ...selectedProject, baseClasses: "CuerpoModificado" })
      }}>
...
// MEJORADO: Ahora simula fielmente las propiedades de edición y borrado de la Solución Completa
vi.mock("./VisualSolutionCodeScreen", () => ({
  VisualSolutionCodeScreen: ({ onBack, onDeleteSection, onUpdateProject, selectedProject }: any) => (
    <div data-testid="screen-visual-solution">
      <button onClick={onBack}>Volver de Solución</button>
      <button onClick={() => onDeleteSection("fullSolution")}>Borrar Solución Completa</button>
      <button onClick={async () => {
        await onUpdateProject({ ...selectedProject, fullSolution: "SolucionModificadaE2E" })
      }}>
...
describe("StorageExamsIndex - Suite Orquestadora Completa", () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks()
    mockChromeStorage = {
      "project_1": { id: "project_1", domainName: "ajedrez", customName: "Examen Mayo", fullSolution: "public class Solucion {}" },
  ...
  describe("Casos Límite y Mutaciones de Estado", () => {
    ...
    // =========================================================================
    // NUEVOS TESTS ADICIONALES PARA MÁXIMA COBERTURA EN VISUAL_SOLUTION
    // =========================================================================
    it("permite borrar la solución completa (fullSolution) desde la pantalla de visualización del código de solución", async () => {
      render(<StorageExamsIndex {...baseProps} />)
      ...
      await act(async () => {
        await userEvent.click(screen.getByRole("button", { name: "Borrar Solución Completa" }))
        await Promise.resolve()
      })

      expect(globalThis.chrome.storage.local.set).toHaveBeenCalledWith(
        expect.objectContaining({
          "project_1": expect.objectContaining({ fullSolution: "" })
        }),
        expect.any(Function)
      )
    })

    it("actualiza y persiste las modificaciones hechas sobre fullSolution utilizando la función onUpdateProject", async () => {
      render(<StorageExamsIndex {...baseProps} />)
      ...
      await act(async () => {
        await userEvent.click(screen.getByRole("button", { name: "Guardar Cambios Solución" }))
        await Promise.resolve()
      })

      expect(globalThis.chrome.storage.local.set).toHaveBeenCalledWith(
        expect.objectContaining({
          "project_1": expect.objectContaining({ fullSolution: "SolucionModificadaE2E" })
        }),
        expect.any(Function)
      )
    })
