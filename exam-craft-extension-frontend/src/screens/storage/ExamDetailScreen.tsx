...
export const ExamDetailScreen: React.FC<ExamDetailScreenProps> = ({
  ...
  const [editingAttributeConstraints, setEditingAttributeConstraints] =
    useState(false)
  const [editingEntityRelationships, setEditingEntityRelationships] =
    useState(false)
  ...
  const liveMermaid =
    parseMermaidFromCombined(combinedText) ||
    selectedProject?.extensionMermaid ||
    ""
      ...
      try {
        ...
        if (!abortRef.current && cleanResult) {
          setCombinedText((prev) =>
            buildCombined(parseIntroFromCombined(prev), cleanResult)
          )
        }
      } catch (err) {
  ...
  const buildUploadListString = () => {
    ...
    if (selectedProject?.testPartsMap)
      items.push("Tests unitarios Java (JUnit)")
  ...
  const repoConfig = (domain = "") => {
    const isPetClinic =
      domain.toLowerCase().includes("veterinaria") ||
      domain.toLowerCase().includes("clínica")
    return {
      TEMPLATE_REPO: isPetClinic
        ...
        <main className="storage-main exam-detail-main">
          ...
          <div className="storage-section-heading">
            ...
            <button
              ...
              onClick={() => setEditingCombined(!editingCombined)}>
            ...
            <div className="content-card">
              ...
              <div className="card-body">
                <textarea
                  ...
                  onChange={
                    editingCombined
                      ? (e) => handleCombinedChange(e.target.value)
                      : undefined
                  }
                />
              </div>
              ...
              <div className="card-header">
                <h3>
                  ...
                  {isRegeneratingDiagram && (
                    <span className="diagram-regenerating-indicator">
                      {" "}
                      Regenerando...
                    </span>
                  )}
              ...
              {selectedProject?.attributeConstraints && (
                <>
                  <button
                    ...
                    onClick={() =>
                      setEditingAttributeConstraints(
                        !editingAttributeConstraints
                      )
                    }>
                    {editingAttributeConstraints
                      ? "✎ Editando"
                  ...
                  <button
                    ...
                    onClick={() =>
                      setSectionToDelete({
                        ...
                        name: "Restricciones de Atributos"
                      })
                    }>
                    ✕
                  </button>
          ...
          <div className="wide-card">
            ...
            {selectedProject?.attributeConstraints ? (
              <textarea
                ...
                onChange={
                  editingAttributeConstraints
                    ? (e) => setAttributeConstraints(e.target.value)
                    : undefined
                }
              />
            ) : (
              <p className="storage-empty-state">
                Aún no se han creado las restricciones de atributos.
              </p>
            )}
          </div>
              ...
              {selectedProject?.entityRelationships && (
                <>
                  <button
                    ...
                    onClick={() =>
                      setEditingEntityRelationships(!editingEntityRelationships)
                    }>
                    {editingEntityRelationships
                      ? "✎ Editando"
                      : "🔒 No editable"}
                  </button>
                  <button
          ...
          <div className="wide-card">
            ...
            {selectedProject?.entityRelationships ? (
              <textarea
                ...
                onChange={
                  editingEntityRelationships
                    ? (e) => setEntityRelationships(e.target.value)
                    : undefined
                }
              />
            ) : (
              <p className="storage-empty-state">
                Aún no se han creado las relaciones entre entidades.
              </p>
            )}
          </div>

          <div
            className="storage-section-heading"
                ...
                <div className="preview-modal-body">
                  <div className="exam-markdown-container">
                    ...
                    <div className="preview-diagram-wrapper">
                      <MermaidViewer
                        chartCode={cleanMermaidCode(liveMermaid)}
                      />
                    </div>
                    <h2>2. Restricciones de Atributos</h2>
                    <p style={{ whiteSpace: "pre-wrap" }}>
                      {attributeConstraints || "Sin restricciones"}
                    </p>
                    <h2>3. Relaciones entre Entidades</h2>
                    <p style={{ whiteSpace: "pre-wrap" }}>
                      {entityRelationships || "Sin relaciones"}
                    </p>
                  </div>
                </div>
              </div>
          ...
          {showDeployModal && (
            <GitHubDeployModal
              ...
              templateRepo={
                repoConfig(selectedProject?.domainName).TEMPLATE_REPO
              }
