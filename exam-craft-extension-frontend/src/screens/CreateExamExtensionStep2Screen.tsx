...
// --- Componentes extraídos para evitar duplicación ---
...
const GenerateButton = ({ onClick, isLoading, label, fullWidth = false }: GenerateButtonProps) => (
  <button
    onClick={onClick}
  ...
  >
    {isLoading ? 'Generando...' : (label ?? 'Generar Diagrama UML')}
  </button>
);
...
// --- Utilidad de limpieza de código Mermaid ---
...
export default function CreateExamExtensionStep2Screen({
  ...
  useEffect(() => {
    if (extensionPromptMarkdown) {
      const { visibleText, hiddenContext: hidden } = parseMasterPrompt(extensionPromptMarkdown);
      setPromptText(visibleText.replaceAll("{{DOMAIN}}", domainName));
      setHiddenContext(hidden);
    }
  }, [domainName]);
      ...
      <header className="app-header">
        <div className="header-left">
          ...
          <BreadcrumbNav
            ...
            onCreateExamByParts={onCreateExamByParts}
            onFunctionalExtension={onFunctionalExtension}
            onStatementStep1={onStatementStep1}
          ...
          <div className="wf-wide-wrapper">
            ...
            {internalStep === 'result' && (
              <div style={{ display: 'flex', gap: '20px', width: '100%', alignItems: 'stretch' }}>
                ...
                {/* Columna 2: Visualización con modal */}
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', flexGrow: 1, minHeight: '200px' }}>
                      {isLoading
                        ...
                        : cleanedCode
                          ...
                          : <span>Esperando código...</span>
                      }
                    </div>
                  </div>
                  <button
                    onClick={() => finishFunctionalExtension(cleanedCode)}
                    className="btn-step primary"
