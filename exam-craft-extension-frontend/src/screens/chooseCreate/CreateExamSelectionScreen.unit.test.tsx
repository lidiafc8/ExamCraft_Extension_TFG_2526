...
vi.mock("../../../assets/images/complete_exam.png", () => ({
  default: "mock-complete-exam-icon"
}))
vi.mock("../../../assets/images/parts_exam.png", () => ({
  default: "mock-parts-exam-icon"
}))
...
describe("CreateExamSelectionScreen – Pruebas Unitarias", () => {
  it("renderiza correctamente los títulos, textos e iconos de la pantalla", () => {
    ...
    expect(
      screen.getByText(/selecciona la modalidad de creación/i)
    ).toBeInTheDocument()
  ...
  it("ejecuta onCreateExamByParts al hacer click en la tarjeta 'Crear examen por partes'", async () => {
    ...
    const btnPartes = screen.getByRole("button", {
      name: /crear examen por partes/i
    })
  ...
  it("no ejecuta ninguna acción al hacer click en la tarjeta deshabilitada 'Crear examen completo'", async () => {
    ...
    const btnCompleto = screen.getByRole("button", {
