...
describe("Integración: SuccessModal", () => {
  ...
  describe("Casos Positivos", () => {
    it("renderiza correctamente el título, el mensaje y el icono de éxito", () => {
      ...
      expect(
        screen.getByRole("heading", { name: "¡Operación Exitosa!", level: 3 })
      ).toBeInTheDocument()
      expect(
        screen.getByText(
          "Los cambios se han guardado correctamente en tu cuenta."
        )
      ).toBeInTheDocument()
    ...
    it("aplica la clase 'secondary' u asigna 'primary' por defecto según la variante", () => {
      const accionesMixtas = [
        ...
        {
          label: "Boton Primario Explicito",
          onClick: vi.fn(),
      ...
      expect(
        screen.getByRole("button", { name: "Boton Secundario" })
      ).toHaveClass("secondary")
      expect(
        screen.getByRole("button", { name: "Boton Primario Explicito" })
      ).toHaveClass("primary")
      expect(
        screen.getByRole("button", { name: "Boton Primario Por Defecto" })
      ).toHaveClass("primary")
    })
  ...
  describe("Casos Límite", () => {
    it("funciona perfectamente y no se rompe si la lista de acciones viene vacía", () => {
      const { container } = render(
        <SuccessModal {...defaultProps} actions={[]} />
      )

      expect(screen.queryByRole("button")).not.toBeInTheDocument()
      expect(
        container.querySelector(".success-modal-actions")
      ).toBeEmptyDOMElement()
    })
