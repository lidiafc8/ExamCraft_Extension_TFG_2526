...
describe("Integración: GitHubDeployModal", () => {
  ...
  describe("Casos Positivos", () => {
    it("renderiza la vista inicial de confirmación con el nombre del repositorio y los elementos formateados", () => {
      ...
      expect(
        screen.getByRole("heading", { name: "CONFIRMAR SUBIDA A GITHUB" })
      ).toBeInTheDocument()
    ...
    it("muestra el campo para ingresar el Token si savedToken es null", () => {
      ...
      expect(
        screen.getByText("Se requiere Token de GitHub:")
      ).toBeInTheDocument()
      expect(
    ...
    it("completa el despliegue exitosamente guardando el token y abriendo el repositorio en otra pestaña", async () => {
      ...
      await waitFor(() => {
        ...
        expect(
          screen.getByRole("heading", { name: "¡Despliegue completado!" })
        ).toBeInTheDocument()
      })
    })
  })
  ...
  describe("Casos Negativos", () => {
    ...
    it("captura los errores del servidor/promesa y renderiza la pantalla de error correspondiente", async () => {
      const customProps = {
        ...
        onConfirm: vi
          .fn()
          .mockRejectedValue(new Error("Token inválido o expirado"))
      }
      ...
      await waitFor(() => {
        expect(
          screen.getByRole("heading", { name: "ERROR EN EL DESPLIEGUE" })
        ).toBeInTheDocument()
        expect(
          screen.getByText(
            /No se pudo crear el repositorio: Token inválido o expirado/i
          )
        ).toBeInTheDocument()
        expect(
  ...
  describe("Casos Límite", () => {
    it("oculta el input de contraseña por completo si savedToken ya viene proveído", () => {
      render(
        <GitHubDeployModal {...defaultProps} savedToken="ghp_token_guardado" />
      )
      ...
      expect(
        screen.queryByText("Se requiere Token de GitHub:")
      ).not.toBeInTheDocument()
    })
    ...
    it("utiliza el mensaje por defecto 'Error' si el objeto de error capturado no contiene la propiedad message (Línea 45)", async () => {
      ...
      await waitFor(() => {
        ...
        expect(
          screen.getByText(/No se pudo crear el repositorio: Error/i)
        ).toBeInTheDocument()
      })
    })
  })
