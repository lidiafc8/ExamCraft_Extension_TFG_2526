...
* Quiero que la extensión funcional evite tratarse de guardería y alojamiento de mascotas

---

## 3. PROPUESTA DEL MODELO
En este ejercicio, añadiremos la funcionalidad de gestión de planes nutricionales personalizados para las mascotas. Concretamente, se proporciona una clase “DietPlan” que representa un plan dietético detallado para una mascota, y que es diseñado por un veterinario. Además, tendremos la clase “DietItem” que representa los alimentos o componentes específicos recomendados dentro de un plan, incluyendo detalles sobre su cantidad y frecuencia. Esta extensión ayudará a los veterinarios a proporcionar una atención nutricional más completa y personalizada.

Las clases para las que realizaremos el mapeo objeto-relacional como entidades JPA son las siguientes:

*   **DietPlan**: Representa un plan nutricional específico diseñado para una mascota. Incluye los atributos `startDate` (la fecha de inicio del plan), `endDate` (la fecha de finalización prevista), `goal` (un texto que describe el objetivo principal del plan, como "pérdida de peso" o "mantenimiento"), `consultationNotes` (notas adicionales del veterinario sobre la consulta) y `dailyCalories` (el total de calorías diarias recomendadas para la mascota).
    *   `Pet` "1" <-- "0..n" `DietPlan`: Un plan dietético es diseñado para una mascota.
    *   `Vet` "1" <-- "0..n" `DietPlan`: Un veterinario prescribe un plan dietético.
    *   `Visit` "0..1" <-- "0..n" `DietPlan`: Un plan dietético puede ser iniciado o revisado en una visita.
    *   `DietPlan` "1" --> "0..n" `DietItem`: Un plan dietético incluye varios ítems de dieta.

*   **DietItem**: Hereda de NamedEntity, por lo que tiene un `name`. Esta clase representa un componente o alimento específico dentro de un plan dietético. Incluye los atributos `quantity` (la cantidad recomendada de este alimento), `unit` (la unidad de medida, como "gramos" o "tazas") y `feedingFrequency` (la frecuencia con la que se debe administrar este alimento, ej. "dos veces al día", "una vez al día").
