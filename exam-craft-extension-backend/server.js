...
const PROVIDERS = {
  gemini: {
    ...
    callApi: async (prompt, key, modelName) => {
      ...
      if (!res.ok)
        throw new Error(
          data.error?.message || "Error desconocido al contactar con Gemini"
        )
  ...
  openai: {
    ...
    callApi: async (prompt, key, modelName) => {
      ...
      if (!res.ok)
        throw new Error(
          data.error?.message || "Error desconocido al contactar con OpenAI"
        )
...
async function executeProvider(providerId, prompt) {
  ...
  if (maxAttempts === 0) {
    throw new Error(
      `No hay API keys configuradas para el proveedor ${providerId}`
    )
  }
...
app.post("/generate", async (req, res) => {
  ...
  for (const providerId of FALLBACK_ORDER) {
    try {
      ...
      console.log(
        `[AI Service] Part successfully generated using provider: ${providerId.toUpperCase()}`
      )

      return res.json({ provider: providerId, text })
    } catch (err) {
      console.error(
        `=> Fallback: Provider '${providerId}' failed. Moving to the next...`
      )
    }
  }
  return res
    .status(500)
    .json({ error: "Todos los servicios de IA están saturados." })
})
...
app.post("/save-log", (req, res) => {
  try {
    ...
    const englishDomain =
      domainTranslations[rawDomain] || rawDomain.replace(/\s+/g, "_")
    ...
    res
      .status(200)
      .json({ message: "Log guardado correctamente en el servidor." })
  } catch (error) {
    console.error(`[Log Service] Failed to save log:`, error)
    res
      .status(500)
      .json({ error: "Error interno al intentar guardar el archivo de log." })
  }
})
...
app.listen(PORT, () => {
  ...
  Object.keys(PROVIDERS).forEach((providerId) => {
    ...
    console.log(
      `- ${providerId.toUpperCase()}: ${numKeys} keys available (Model: ${provider.model})`
    )
  })
