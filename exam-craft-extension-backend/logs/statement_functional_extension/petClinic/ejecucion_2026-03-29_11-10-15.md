...
**Dominio:** PETCLINIC
**Fecha:** 29/3/2026, 13:10:15

## Índice
- [1. Hidden Context](#1-hidden-context)
- [2. Previous Extensions](#2-previous-extensions)
- [3. Visible Prompt](#3-visible-prompt)
...
--- EXTENSIÓN FUNCIONAL PREVIA 1 (Vacunas) ---
En este ejercicio, añadiremos la funcionalidad de gestión de programas de vacunación para las mascotas.

En concreto, tendremos la clase `Vaccine`, que representa las diferentes vacunas disponibles, y que contiene información como su descripción, el fabricante y el número de meses que la vacuna es efectiva. Además, tendremos la clase `VaccinationRecord`, que se encargará de registrar cada vez que una mascota reciba una vacuna, indicando la fecha en que se administró, la fecha en que la próxima dosis o revisión es debida, y posibles notas adicionales sobre la vacunación.

Cada `Vaccine` puede ser aplicada a uno o varios tipos de mascotas. Las `VaccinationRecord`s se asocian con la mascota a la que se le administra la vacuna y con la `Vaccine` específica utilizada. También se registra qué veterinario realizó la administración de la vacuna.
...
Pet "0..n" --> "1" PetType
Pet "0..n" --> "1" Visit
Vaccine "0..n" --> "1..n" PetType : can be applied to
VaccinationRecord "0..n" --> "1" Pet : administered to
VaccinationRecord "0..n" --> "1" Vaccine : uses
VaccinationRecord "0..n" --> "1" Vet : administered by

--- EXTENSIÓN FUNCIONAL PREVIA 2 (Alojamientos) ---
En este ejercicio, añadiremos la funcionalidad de gestión de estancias de mascotas en la clínica (servicio de guardería o "boarding").

Concretamente, tendremos la clase “BoardingStay”, que representa el registro de la estancia de una mascota en la clínica. Esta clase incluirá la fecha de inicio (startDate), la fecha de fin (endDate), la tarifa diaria (dailyRate) y cualquier instrucción especial de cuidado (specialInstructions) para la mascota durante su estancia. Cada estancia estará asociada a una única mascota. Además, tendremos la clase “Cage” que representa cada una de las jaulas o espacios individuales disponibles en la clínica para alojar a las mascotas. La clase Cage contendrá su identificador único como nombre, su tamaño (size) y un indicador de si está disponible (isAvailable). Una estancia concreta se realizará en una única jaula, y una jaula podrá albergar múltiples estancias a lo largo del tiempo.
...
Pet "0..n" --> "1" PetType
Pet "1" <-- "0..n" Visit
BoardingStay "0..n" --> "1" Pet
BoardingStay "0..n" --> "1" Cage

--- EXTENSIÓN FUNCIONAL PREVIA 3 (Dietas) ---
En este ejercicio, añadiremos la funcionalidad de gestión de planes de alimentación y recomendaciones de comida para las mascotas. Concretamente, tendremos la clase “DietPlan”, que representa un plan de dieta específico para una mascota, incluyendo información sobre su descripción, fechas de inicio y fin, la ingesta calórica diaria recomendada y notas adicionales. Este plan será diseñado por un veterinario para una mascota en particular. Además, tendremos la clase “FoodRecommendation”, que representa una recomendación de alimento particular dentro de un plan de dieta. Contendrá detalles como el nombre de la marca, el tipo de alimento (por ejemplo, "seco", "húmedo"), la cantidad diaria en gramos y la frecuencia de alimentación. Cada recomendación de alimento formará parte de un plan de dieta.
...
%% --- Nuevas Asociaciones y Relaciones ---
Vet "1" --> "0..n" DietPlan : designs >
Pet "1" <-- "0..n" DietPlan : for >
DietPlan "1" <-- "0..n" FoodRecommendation : includes >
...
## 4. Response
En este ejercicio, añadiremos la funcionalidad de gestión de pruebas de laboratorio y sus resultados para las mascotas. Concretamente, se proporciona una clase “LabTest” que representa los distintos tipos de pruebas de laboratorio que se pueden realizar en la clínica (por ejemplo, "análisis de sangre", "cultivo de orina", "biopsia"). Esta clase incluirá una descripción detallada de la prueba, su coste asociado y el tipo de muestra biológica necesaria para realizarla.

Además, tendremos la clase “LabResult” que representa el resultado de una prueba de laboratorio realizada a una mascota durante una visita veterinaria. Esta clase contendrá la fecha en que se obtuvo el resultado, un resumen de los hallazgos, un valor numérico (si aplica para pruebas cuantitativas) junto con su unidad de medida, y un indicador de si el resultado se considera anormal. Un resultado de laboratorio estará siempre asociado a una única prueba de laboratorio y se registrará para una mascota específica, habiendo sido solicitado en el contexto de una visita particular.
