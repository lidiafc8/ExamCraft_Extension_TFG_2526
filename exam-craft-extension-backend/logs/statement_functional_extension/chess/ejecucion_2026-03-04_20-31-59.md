...
**Ejercicio:** FUNCTIONAL_EXTENSION
**Dominio:** CHESS
**Fecha:** 4/3/2026, 21:31:59
...
En este ejercicio, añadiremos la funcionalidad de gestión de logros y trofeos para una implementación del juego del ajedrez. Concretamente, se creará un sistema de "Achievements" que los usuarios podrán desbloquear al realizar acciones específicas en sus partidas de ajedrez.

Tendremos las clases “Achievement”, que representa un logro o meta específica que los jugadores pueden alcanzar. Esta clase incluye una `description` para detallar el logro, un `targetValue` que es el valor numérico a alcanzar para completarlo (por ejemplo, 10 partidas ganadas), y un `achievementType` que especifica la categoría del objetivo (por ejemplo, 'WIN_GAMES', 'CAPTURE_PIECES', 'CHECKMATE_VICTORIES'). Para registrar la situación de cada jugador respecto a los logros, se crea la clase “UserAchievement”, que guarda el `currentProgress` de un usuario para un logro determinado y la `achievedDate` en la que el logro fue finalizado, si ya se ha completado. Cada `UserAchievement` estará asociada a un único usuario (User) y a un único `Achievement`.

Las clases para las que realizaremos el mapeo objeto-relacional como entidades JPA se detallan en el diagrama UML adjunto. Realizaremos una serie de ejercicios basados en funcionalidades que implementaremos en el sistema, y validaremos mediante pruebas unitarias.