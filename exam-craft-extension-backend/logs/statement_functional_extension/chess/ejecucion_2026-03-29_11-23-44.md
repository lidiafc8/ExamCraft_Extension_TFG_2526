...
# EXTENSIÓN FUNCIONAL PREVIA 1 (Puzzle)
En este ejercicio, añadiremos la funcionalidad de gestión de puzzles de ajedrez y el registro de los intentos de los usuarios para resolverlos. Concretamente, tendremos la clase “Puzzle” que representa un problema o ejercicio de ajedrez, el cual viene definido por una posición del tablero en formato FEN (Forsyth-Edwards Notation), los movimientos que constituyen su solución, y un nivel de dificultad para su resolución.

Además, tendremos la clase “PuzzleAttempt” que registrará cada intento que un usuario haga para resolver un puzzle. Esta clase incluirá la fecha y hora en la que se realizó el intento, si el puzzle fue resuelto con éxito por el usuario y el tiempo invertido en segundos para su resolución. Cada intento estará asociado a un único puzzle y a un único usuario del sistema.
...
ChessBoard "1" *--> "0..n" Piece
Authorities "0..n" -- "" User
Puzzle "1" <-- "0..n" PuzzleAttempt
User "1" <-- "0..n" PuzzleAttempt
...
## 4. Response
En este ejercicio, añadiremos la funcionalidad de gestión de análisis de partidas de ajedrez y anotaciones específicas de movimientos. Concretamente, se proporciona una clase “ChessMatch” que representa las partidas que se juegan, y un conjunto de usuarios que pueden realizar análisis.

Además, tendremos las clases “GameAnalysis”, que representa un análisis detallado que un usuario puede realizar sobre una partida de ajedrez previamente jugada. Incluirá un título descriptivo, la fecha en que se realizó el análisis y una evaluación general de la partida. También tendremos la clase “MoveAnnotation”, que permite a los usuarios añadir comentarios o evaluaciones específicas a movimientos concretos dentro de un análisis de partida, indicando el número de movimiento, el comentario y el tipo de evaluación aplicada (por ejemplo, "error grave" o "jugada brillante").
