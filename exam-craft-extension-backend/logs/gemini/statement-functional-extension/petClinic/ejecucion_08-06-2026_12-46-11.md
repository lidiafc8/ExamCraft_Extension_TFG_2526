...
## 4. Respuesta
En este ejercicio, añadiremos la funcionalidad de gestión de servicios de alojamiento y guardería para mascotas en la clínica. Concretamente, tendremos la clase “BoardingStay”, que representa el período de tiempo específico en que una mascota se hospeda en las instalaciones de la clínica. Además, tendremos la clase “AccommodationUnit”, que representa las diferentes unidades físicas (como jaulas o habitaciones) disponibles para el alojamiento de las mascotas.

La clase BoardingStay contendrá información como la fecha de entrada (`checkInDate`), la fecha de salida (`checkOutDate`), el coste diario del servicio (`dailyCost`), cualquier nota especial sobre el cuidado de la mascota durante su estancia (`specialCareNotes`) y su estado actual (`status`). Por su parte, la clase AccommodationUnit identificará cada espacio físico con un nombre único (`name`), su tamaño en metros cuadrados (`sizeSquareMeters`), la capacidad máxima de mascotas que puede albergar (`maxCapacityPets`) y características adicionales relevantes (`features`).

Cada estancia de alojamiento se asociará con la mascota que se está hospedando y puede ser supervisada por un veterinario específico de la clínica. A su vez, cada estancia se asignará a una unidad de alojamiento disponible.
