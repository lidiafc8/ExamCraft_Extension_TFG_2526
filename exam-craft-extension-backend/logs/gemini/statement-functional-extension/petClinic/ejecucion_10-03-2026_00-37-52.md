...
En este ejercicio, añadiremos la funcionalidad de gestión de servicios de residencia o guardería para mascotas. Concretamente, tendremos la clase “BoardingStay” que representa la estancia de una mascota en la clínica para servicios de residencia. Esta clase registra la fecha de entrada (`checkInDate`) y salida (`checkOutDate`), así como cualquier requisito especial (`specialRequirements`) y el coste final (`finalCost`) de la estancia. Además, tendremos la clase “Kennel” que representa las distintas jaulas o habitaciones disponibles en la clínica para alojar a las mascotas. Un Kennel tiene un tamaño (`size`), una capacidad (`capacity`) para mascotas y un precio base diario (`dailyPrice`). Cada estancia se realiza para una mascota específica, y se asigna a una jaula disponible, la cual, a su vez, está diseñada principalmente para un tipo de mascota.
