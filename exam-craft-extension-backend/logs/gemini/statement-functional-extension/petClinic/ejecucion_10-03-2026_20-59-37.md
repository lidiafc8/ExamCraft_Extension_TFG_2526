...
En este ejercicio, añadiremos la funcionalidad de gestión de alojamiento de mascotas (pet boarding) en la clínica. Concretamente, tendremos la clase “Boarding”, que representa la reserva y el registro de la estancia de una mascota en las instalaciones de alojamiento. Esta clase incluye atributos como la fecha de entrada (`checkInDate`), la fecha de salida (`checkOutDate`), el coste diario del alojamiento (`dailyCost`) y cualquier instrucción especial para el cuidado de la mascota (`specialInstructions`). Además, tendremos la clase “BoardingUnit”, que representa las distintas unidades de alojamiento disponibles en la clínica, como jaulas o habitaciones. Esta clase incluye atributos como el tamaño de la unidad (`size`), si está climatizada (`isClimateControlled`) y la capacidad máxima de mascotas (`maxCapacity`). Una mascota puede tener múltiples estancias de alojamiento a lo largo del tiempo, y cada estancia de `Boarding` se asigna a una `BoardingUnit` específica. La clase `Boarding` también se relaciona con el `Owner` que realiza la reserva de alojamiento para su mascota.
