...
# EXTENSIÓN FUNCIONAL PREVIA 1 (Examen de Ajedrez)
En este ejercicio, añadiremos la funcionalidad de gestión de análisis de partidas y retroalimentación de los jugadores para una implementación del juego del ajedrez. Concretamente, se proporciona una clase “ChessMatch” que representa las partidas que se juegan, y que tiene asociada una instancia de la clase “ChessBoard” que representa el estado del tablero para dicha partida, por lo que tendrá asociado un conjunto de instancias de la clase “Piece”.

Además, tendremos las clases “MatchReview”, que representa una valoración y comentarios sobre una partida por parte de un jugador, con atributos como la `rating` (una puntuación numérica), el `comment` (texto descriptivo) y la `reviewDate` (fecha en que se realizó la valoración). Esta clase se relaciona con una partida de ajedrez específica y es realizada por un usuario del sistema.

También tendremos la clase “MatchAnalysisPoint”, que registra puntos clave durante una partida para su posterior análisis, incluyendo el `turnNumber` (número del turno en la partida), el `type` (tipo de punto de análisis, como "Error" o "Movimiento Brillante") y una `description` (detalle del punto de análisis). Un punto de análisis se asocia a una partida de ajedrez concreta y, opcionalmente, a un usuario que lo crea.
...
ChessBoard "1" * --> "0..n" Piece
Authorities "0..n" --> "1" User
MatchReview "0..n" --> "1" ChessMatch
MatchReview "0..n" --> "1" User: reviewedBy
MatchAnalysisPoint "0..n" --> "1" ChessMatch
MatchAnalysisPoint "0..n" --> "0..1" User: createdBy
```

# EXTENSIÓN FUNCIONAL PREVIA 2 (33)
En este ejercicio, añadiremos la funcionalidad de gestión de puzles de ajedrez y su resolución para una implementación del juego del ajedrez. Concretamente, se proporciona una clase “ChessMatch” que representa las partidas que se juegan, y que tiene asociada una instancia de la clase “ChessBoard” que representa el estado del tablero para dicha partida, por lo que tendrá asociada un conjunto de instancias de la clase “Piece”.

Además, tendremos la clase “ChessPuzzle”, que representa un puzle de ajedrez con un estado de tablero inicial. Incluye atributos como la `difficulty` (un nivel de dificultad numérico), un `theme` (que describe la naturaleza del puzle, como "Mate en 2" o "Táctica de Horquilla"), una `fenPosition` que representa la posición de las piezas en el tablero en formato FEN, y la `solution` (la secuencia de movimientos correcta para resolverlo). Los puzles pueden ser creados por usuarios del sistema.

También tendremos la clase “PuzzleAttempt”, que registra el intento de un usuario de resolver un puzle específico. Esta clase contiene la `attemptDate` (fecha en que se realizó el intento), un indicador `isSolved` (booleano que indica si el puzle fue resuelto correctamente), los `movesMade` por el usuario durante el intento, y el `timeTakenSeconds` (el tiempo que el usuario invirtió en resolverlo). Un intento de puzle se asocia a un puzle de ajedrez concreto y al usuario que realizó dicho intento.
...
ChessPuzzle "1" * --> "1" ChessBoard : <font color=red>initialBoard</font>
User "1" --> "0..n" ChessPuzzle : <font color=red>creates</font>
PuzzleAttempt "0..n" --> "1" ChessPuzzle : <font color=red>attempts</font>
PuzzleAttempt "0..n" --> "1" User : <font color=red>madeBy</font>
%% Style
style ChessPuzzle stroke:red,color:red
style PuzzleAttempt stroke:red,color:red
```

# EXTENSIÓN FUNCIONAL PREVIA 3 (Exam``)
En este ejercicio, añadiremos la funcionalidad de gestión de aperturas y el seguimiento de su rendimiento para una implementación del juego del ajedrez. Concretamente, se proporciona una clase “ChessMatch” que representa las partidas que se juegan, y que tiene asociada una instancia de la clase “ChessBoard” que representa el estado del tablero para dicha partida, por lo que tendrá asociada un conjunto de instancias de la clase “Piece”.

Además, tendremos la clase “ChessOpening”, que representa una apertura de ajedrez específica, incluyendo su `description` (texto descriptivo), el `ecoCode` (código estándar ECO, por ejemplo, "Ruy Lopez C60") y la `mainLineMoves` (la secuencia de movimientos que define la línea principal de la apertura). Una partida de ajedrez se relacionará con la apertura utilizada.

También tendremos la clase “OpeningPerformance”, que registra el rendimiento de un usuario con una `ChessOpening` particular. Esta clase contiene el `playedCount` (número de veces que el usuario ha jugado esta apertura), `winCount`, `lossCount`, `drawCount` y el `averageEloChange` (el cambio promedio en la calificación ELO del usuario al jugar esta apertura). Esta clase se asocia al usuario que la posee y a la `ChessOpening` a la que se refiere.
...
ChessBoard "1" * --> "0..n" Piece
Authorities "0..n" -- User
ChessMatch "0..n" --> "1" ChessOpening : uses
User "1" --> "0..n" OpeningPerformance : hasPerformance
OpeningPerformance "0..n" --> "1" ChessOpening : <font color=red>forOpening</font>
%% Style
style ChessOpening stroke:red,color:red
style OpeningPerformance stroke:red,color:red
```

# EXTENSIÓN FUNCIONAL PREVIA 4 (Prueba 3)
...
# EXTENSIÓN FUNCIONAL PREVIA 5 (Prueba 4)
