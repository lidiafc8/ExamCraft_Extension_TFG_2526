...
En este ejercicio, añadiremos la funcionalidad de gestión de logros y recompensas para los usuarios del juego del ajedrez. Concretamente, tendremos la clase "Achievement", que representa un reconocimiento o una meta alcanzada por los jugadores. Cada `Achievement` tiene una descripción que detalla el logro, y un valor en `points` que se suma al perfil del usuario. Además, tendremos la clase "UserAchievement" que registra la fecha (`achievedDate`) en que un jugador ha conseguido un logro específico y una breve `criteriaMet` que explica cómo se ha cumplido dicho logro. La clase `UserAchievement` se relaciona con `User` para indicar qué jugador ha conseguido el logro, y con `Achievement` para especificar cuál logro ha sido obtenido.
