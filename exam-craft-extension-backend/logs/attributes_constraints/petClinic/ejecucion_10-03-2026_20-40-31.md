...
Aquí tienes el ejercicio para el examen, con las restricciones de atributos de las nuevas entidades, siguiendo el formato y las directrices proporcionadas:

```
Modificar las clases “TrainingCourse” y “Enrollment” para que sean entidades. Estas clases están alojadas en el paquete “org.springframework.samples.petclinic.training”, y deben tener los siguientes atributos y restricciones:
```

**Para la clase TrainingCourse:**
...
- Un atributo de tipo cadena de caracteres (String) llamado “name” obligatorio (no puede ser nulo), que debe tener una longitud mínima de 5 caracteres y máxima de 100 y que no puede estar formada por caracteres vacíos (espacios, tabuladores, etc.).
- El atributo de tipo cadena de caracteres (String) llamado “description” obligatorio, que debe tener una longitud mínima de 20 caracteres y máxima de 500, y no puede estar formada por caracteres vacíos.
- El atributo de tipo entero (Integer) llamado “durationHours” obligatorio, que representa la duración total del curso en horas. Este atributo tendrá un valor mínimo de 1 hora y un valor máximo de 200 horas.
- El atributo de tipo doble (Double) llamado “cost” obligatorio, que representa el precio de matriculación del curso. Este atributo tendrá un valor mínimo de 0.0 (permitiendo cursos gratuitos) y un valor máximo de 5000.0, con dos decimales de precisión.
- El atributo de tipo entero (Integer) llamado “maxCapacity” obligatorio, que representa el número máximo de mascotas que pueden inscribirse en el curso. Este atributo tendrá un valor mínimo de 1 mascota y un máximo de 100.

**Para la clase Enrollment:**

- El atributo de tipo entero (Integer) llamado “id” actuará como clave primaria en la tabla de la base de datos relacional asociada a la entidad.
- El atributo de tipo fecha (LocalDate) llamado “enrollmentDate” obligatorio, que representa la fecha en la que se realizó la inscripción de la mascota en el curso. Seguirá el formato “dd/MM/yyyy”.
- El atributo de tipo cadena de caracteres (String) llamado “status” obligatorio, que representa el estado actual de la participación de la mascota en el curso. Únicamente podrá tomar los valores “PENDING”, “ACTIVE”, “COMPLETED”, “DROPPED”.
- El atributo de tipo entero (Integer) llamado “finalGrade” opcional, que representa la calificación final obtenida por la mascota en el curso. Si toma valor, debe estar en el rango de 0 a 10, ambos inclusive.

```
No modifique por ahora las anotaciones @Transient de las clases. Modificar las interfaces “TrainingCourseRepository” y “EnrollmentRepository” alojada en el mismo paquete para que extienda a CrudRepository.
```